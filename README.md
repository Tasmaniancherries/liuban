...
其他（DTO／session／設定）：`test/data_models_dto_test.dart`（各資料模型 `fromJson`／`listFromResponse`）、`test/app_session_test.dart`（`AppSession` 階段與通知）、`test/post_audience_test.dart`（`PostAudience` 與 API 字串）、`test/app_config_test.dart`（`AppConfig` 預設編譯期常數）、`test/app_container_test.dart`（`AppContainer` DI 與 baseUrl wiring）、`test/promotion_models_test.dart`（`PromotionItem.fromDto`／`promotionById`）。
