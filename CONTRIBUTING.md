...
`analysis_options.yaml` enables Dart **`strict-inference`** and **`strict-casts`**, plus extra lints such as **`prefer_single_quotes`**, **`require_trailing_commas`**, **`directives_ordering`**, **`use_super_parameters`**, Flutter helpers (**`sized_box_for_whitespace`**, **`use_colored_box`**, etc.), **`prefer_final_locals`**, **`unawaited_futures`** / **`discarded_futures`** (use **`await`**, **`unawaited()`**, or **`unawaitedDebug` / `unawaitedDebugFuture`**), **`avoid_print`** (prefer **`debugPrint`** or proper logging), **`sort_pub_dependencies`** / **`depend_on_referenced_packages`** (keep **`pubspec.yaml`** tidy), and small clarity rules (**`avoid_redundant_argument_values`**, **`use_raw_strings`**, **`exhaustive_cases`**, …). Prefer explicit type arguments when the analyzer reports `inference_failure_*` rather than weakening types. Mechanical cleanups often clear with **`dart fix --apply`** before you hand-edit. Overrides should be marked with **`@override`** (**`annotate_overrides`**); prefer **`rethrow`** over **`throw e`** in catch blocks when appropriate (**`use_rethrow_when_possible`**). Widget tests **`test/widget_test.dart`** also assert **bottom navigation tooltips** (`NavigationDestination` copy in `MainShell`) so tightened icon labels without updated tooltips fail early.
...
PRs are auto-labeled by `.github/workflows/pr-labeler.yml` + `.github/labeler.yml` based on changed paths. Labels (`ci`, `docs`, `dependencies`, `android`, `ios`, `flutter`) are auto-created/updated by `.github/workflows/labels-sync.yml` (manual trigger or when label config changes on `main`).
