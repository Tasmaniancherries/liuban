...
`analysis_options.yaml` enables Dart **`strict-inference`** and **`strict-casts`**, plus extra lints such as **`prefer_single_quotes`**, **`require_trailing_commas`**, **`directives_ordering`**, **`use_super_parameters`**, Flutter helpers (**`sized_box_for_whitespace`**, **`use_colored_box`**, etc.), **`prefer_final_locals`**, **`unawaited_futures`** / **`discarded_futures`** (use **`await`**, **`unawaited()`**, or **`unawaitedDebug` / `unawaitedDebugFuture`**), **`avoid_print`** (prefer **`debugPrint`** or proper logging), and **`sort_pub_dependencies`** / **`depend_on_referenced_packages`** (keep **`pubspec.yaml`** tidy). Prefer explicit type arguments when the analyzer reports `inference_failure_*` rather than weakening types. Mechanical cleanups often clear with **`dart fix --apply`** before you hand-edit.
